...
const AZTEC_ARCHIVAL_NODE_URL = process.env.NEXT_PUBLIC_AZTEC_ARCHIVAL_NODE_URL;

export async function POST(request: NextRequest) {
  try {
    if (!AZTEC_ARCHIVAL_NODE_URL) {
      throw new Error("NEXT_PUBLIC_AZTEC_ARCHIVAL_NODE_URL is required for audit event retrieval");
    }
